...
export class YamlRename {
  ...
  private findAnchorToken(yamlDoc: SingleYAMLDocument, node: Node): CST.SourceToken | undefined {
    ...
    const candidates = [];
    if (parent && (parent as unknown as { srcToken?: CST.SourceToken }).srcToken) {
      candidates.push((parent as unknown as { srcToken: CST.SourceToken }).srcToken);
    }
    if ((node as unknown as { srcToken?: CST.SourceToken }).srcToken) {
      candidates.push((node as unknown as { srcToken: CST.SourceToken }).srcToken);
    }
  ...
  private getAnchorFromToken(token: CST.SourceToken, node: Node): CST.SourceToken | undefined {
    if (isCollectionItem(token)) {
      ...
      for (const item of collection.items ?? []) {
        if (item.value !== (node as unknown as { srcToken?: CST.SourceToken }).srcToken) {
          continue;
        }
