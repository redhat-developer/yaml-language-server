...
export class ValidationHandler {
  ...
  validateTextDocument(textDocument: TextDocument): Promise<Diagnostic[]> {
    ...
    return this.languageService.doValidation(textDocument, isKubernetesAssociatedDocument(textDocument, this.yamlSettings.specificValidatorPaths)).then((diagnosticResults) => {
      const diagnostics = [];
